...
var Index009=React.createClass({
  ...
  getUserList: function(flag){
    ...
    if(flag){
      $.ajax({
        ...
        success: function(data) {
          ...
          if(data.c == 1000){
            ...
            //缓存菜单数据
            sessionStorage.setItem('menu_info_index009',JSON.stringify(data.ntl));
          }
        }.bind(this),
     ...
     var temp = this.checkMenuType(JSON.parse(sessionStorage.getItem('menu_info_index009')));
     if(temp.length > 1 && temp.length < 4){
        var  arr0 = []
        for(var i=0;i<3;i++){
          arr0.push(temp[i]);
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data.c == 1000){
          ...
          }else if(sessionStorage.getItem('menu_version_index009')){
            if(sessionStorage.getItem('menu_version_index009') != data.mv){
              this.getUserList(true);
            }else{
          ...
          sessionStorage.setItem('menu_version_index009',data.mv);
        }
      }.bind(this),
      error: function(xhr, status, err) {
        this.showAlert('系统开了小差，请刷新页面');
