...
require('../../../../css/theme/theme014.less');
var Index014 = React.createClass({
	mixins:[CommonMixin],
  ...
  getUserList: function(flag){
    ...
    if(flag){
      $.ajax({
        ...
        success: function(data) {
          ...
          if(data.c == 1000){
            ...
            //缓存菜单数据
            sessionStorage.setItem('menu_info_index014',JSON.stringify(data.ntl));
          }
        }.bind(this),
      ...
      var localJsons = this.checkMenuType(JSON.parse(sessionStorage.getItem('menu_info_index014')));
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data.c == 1000){
          ...
          sessionStorage.setItem('menu_version_index014',data.mv);
        }
      }.bind(this),
      error: function(xhr, status, err) {
        this.showAlert('系统开了小差，请刷新页面');
  ...
  componentWillMount: function(){
    ...
    var navNodes = this.state.navArrs.map(function(item,i){
      return(
		...
		return (
				<div>
					<div className="theme6_main">
							...
							<ul className="theme14_main_list">
                {navNodes}
							</ul>
						</div>
        ...
        imgUrl={global.img+this.state.shareImg} target="index014"/>
        <Message/>
