...
require('../../../../css/theme/theme021.less');

var Index021 = React.createClass({
	mixins:[CommonMixin],
  ...
  getUserList: function(flag){
    ...
    if(flag){
      $.ajax({
        ...
        success: function(data) {
          ...
          if(data.c == 1000){
            ...
            //缓存菜单数据
            sessionStorage.setItem('menu_info_index021',JSON.stringify(data.ntl));
          }
        }.bind(this),
        error: function(xhr, status, err) {
          this.showRefresh('系统开了小差，请刷新页面');
      ...
      var localJsons = this.checkMenuType(JSON.parse(sessionStorage.getItem('menu_info_index021')));
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data.c == 1000){
          ...
          }else if(sessionStorage.getItem('menu_version_index021')){
            if(sessionStorage.getItem('menu_version_index021') != data.mv){
              this.getUserList(true);
            }else{
          ...
          sessionStorage.setItem('menu_version_index021',data.mv);
        }
      }.bind(this),
      error: function(xhr, status, err) {
        this.showRefresh('系统开了小差，请刷新页面');
  ...
  componentWillMount: function(){
    ...
    var navNodes = this.state.navArrs.map(function(item,i){
      return(
		...
		return (
				<div className="theme021">
          <div className="theme021_blur"></div>
            ...
            <ul className="theme021_menu_list">
              {navNodes}
            </ul>
          </div>
					<Share title={this.state.shareTitle} desc={this.state.shareDesc} 
          imgUrl={global.img+this.state.shareImg} target="index021"/>
          <Message/>
