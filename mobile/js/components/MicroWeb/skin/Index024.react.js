...
require('../../../../css/theme/theme024.less');

var Index024 = React.createClass({
	mixins:[CommonMixin],
  ...
  getUserList: function(flag){
    ...
    if(flag){
      $.ajax({
        ...
        success: function(data) {
          ...
          if(data.c == 1000){
            ...
            //缓存菜单数据
            sessionStorage.setItem('menu_info_index024',JSON.stringify(data.ntl));
          }
        }.bind(this),
        error: function(xhr, status, err) {
          this.showRefresh('系统开了小差，请刷新页面');
      ...
      var localJsons = this.checkMenuType(JSON.parse(sessionStorage.getItem('menu_info_index024')));
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data.c == 1000){
          ...
          }else if(sessionStorage.getItem('menu_version_index024')){
            if(sessionStorage.getItem('menu_version_index024') != data.mv){
              this.getUserList(true);
            }else{
          ...
          sessionStorage.setItem('menu_version_index024',data.mv);
        }
      }.bind(this),
      error: function(xhr, status, err) {
        this.showRefresh('系统开了小差，请刷新页面');
    ...
    var navNodes = this.state.navArrs.map(function(item,i){
      return(
		...
		return (
          ...
          imgUrl={global.img+this.state.shareImg} target="index024"/>
          <Message/>
