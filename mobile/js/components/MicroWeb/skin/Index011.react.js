...
require('../../../../css/theme/theme011.less');
var Index011 = React.createClass({
  ...
  getUserList: function(flag){
    ...
    if(flag){
      $.ajax({
        ...
        success: function(data) {
          ...
          if(data.c == 1000){
            ...
            //缓存菜单数据
            sessionStorage.setItem('menu_info_index011',JSON.stringify(data.ntl));
          }
        }.bind(this),
      ...
      var localJsons = this.checkMenuType(JSON.parse(sessionStorage.getItem('menu_info_index011')));
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data.c == 1000){
          ...
          }else if(sessionStorage.getItem('menu_version_index011')){
            if(sessionStorage.getItem('menu_version_index011') != data.mv){
              this.getUserList(true);
            }else{
          ...
          sessionStorage.setItem('menu_version_index011',data.mv);
        }
      }.bind(this),
      error: function(xhr, status, err) {
        this.showAlert('系统开了小差，请刷新页面');
  ...
  componentWillMount: function(){
    ...
    var navNodes = this.state.navArrs.map(function(item,i){
      return(
        ...
        imgUrl={global.img+this.state.shareImg} target="index011"/>
        <Message/>
