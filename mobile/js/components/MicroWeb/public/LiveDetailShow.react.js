...
var LiveDetailShow = React.createClass({
  ...
  render:function(){
      ...
      return(
        <div key={i}>
          <div className="live_detail_class" style={{display:item.bdid?"block":"none"}}>
            ...
            <div className="live_detail_openbaiduoffice" onClick={this.openBaiduOffice.bind(this,item.bdid)}>{item.dn}<span className="live_detail_introduce_box_span">{this.state.openBaiduOfficeText}</span></div>
            <div id="reader"></div>
          </div>
    ...
    var ShareBox = this.state.shareData.map(function(item,i){
      return(
        <Share key={i} title={item.lt?item.lt:this.state.liveListTitle} desc={item.lt?(item.sn+'关于'+item.lt+'的精彩直播'):this.state.liveListTitle} imgUrl={item.sp?(global.img+item.sp):(global.img+this.state.liveListPic)} target="LiveDetailShow" ldid={item.ldid?item.ldid:'0'}/>
      )
    }.bind(this));
